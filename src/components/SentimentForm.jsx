...
export default function SentimentForm({ token, employeeId = null }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const url = employeeId ? `/api/employees/${encodeURIComponent(employeeId)}/status` : '/api/employees/me/status';
  ...
  useEffect(() => {
    ...
    const url = `/api/sentiment/form/${formId}?t=${Date.now()}`; // cache-busting
    fetch(url, {
