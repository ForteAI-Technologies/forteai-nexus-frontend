    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (newCompanyHR?.employeesID) {
          try {
            const hrResp = await fetch(`/api/admin/company/${encodeURIComponent(newCompany.company)}/hr`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', Authorization: token ? `Bearer ${token}` : '' },
...
function CompanyList({ token, onSelect, onDeselect }) {
  ...
  useEffect(() => {
    ...
    const fetchCompanies = async () => {
      ...
      setError(null);
      try {
        const res = await fetch('/api/admin/companies', { headers: { Authorization: token ? `Bearer ${token}` : '' } });
      ...
      const filterCompanies = async () => {
        ...
        await Promise.all(
          companies.map(async (c) => {
            ...
            try {
              const res = await fetch(`/api/admin/company/${encodeURIComponent(name)}/employees`, { headers: { Authorization: token ? `Bearer ${token}` : '' } });
              if (res.ok) {
                const data = await res.json();
  ...
  useEffect(() => {
    if (companies.length > 0) {
      companies.forEach((c) => {
        const name = c.company || c;
        fetch(`/api/admin/company/${encodeURIComponent(name)}/feedback`, { headers: { Authorization: token ? `Bearer ${token}` : '' } })
          .then(res => res.json())
          .then(data => {
  ...
  useEffect(() => {
    companiesToShow.forEach(async (c) => {
      const name = c.company || c;
      try {
        const res = await fetch(`/api/admin/company/${encodeURIComponent(name)}/report/status`, { headers: { Authorization: token ? `Bearer ${token}` : '' } });
  ...
  const fetchFeedback = async (company) => {
    ...
    try {
      const res = await fetch(`/api/admin/company/${encodeURIComponent(company)}/feedback`, { headers: { Authorization: token ? `Bearer ${token}` : '' } });
      const data = await res.json();
  ...
  const openCompany = async (company) => {
    if (!employeesByCompany[company]) {
      try {
        const res = await fetch(`/api/admin/company/${encodeURIComponent(company)}/employees`, {
    ...
    try {
      const statusRes = await fetch(`/api/admin/company/${encodeURIComponent(company)}/report/status`, {
        headers: { Authorization: token ? `Bearer ${token}` : '' }
      });
      ...
      if (statusRes.ok && statusData.filled > 0) {
        ...
        const repRes = await fetch(`/api/admin/company/${encodeURIComponent(company)}/report`, {
          headers: { Authorization: token ? `Bearer ${token}` : '' }
        });
