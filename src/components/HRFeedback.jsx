...
const HRFeedback = ({ token, onComplete }) => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      let already = false;
      try {
        const respStatus = await fetch('/api/hr/feedback/responses', {
      ...
      // Fetch questions
      try {
        const resp = await fetch('/api/hr/feedback/questions', {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const resp = await fetch('/api/hr/feedback/responses', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
